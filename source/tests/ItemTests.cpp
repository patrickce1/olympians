...
void testEffectiveValueComputation(const std::shared_ptr<cugl::JsonValue>& itemsJson,
    ...
    shieldTarget.updateHealth(-6.0f);
        assertWithLabel(floatsEqualWithinTolerance(shieldTarget.getCurrentHealth(), shieldedHealthBefore - 3.0f), "compute: shield mitigates fixed damage from the next hit");
        assertWithLabel(!shieldTarget.hasShield(), "compute: shield is consumed after blocking one hit");
    ...
    barrierTarget.updateHealth(-6.0f);
        assertWithLabel(floatsEqualWithinTolerance(barrierTarget.getCurrentHealth(), barrierHealthBefore - 3.0f), "compute: barrier mitigates the next hit by percentage");
        assertWithLabel(!barrierTarget.hasBarrier(), "compute: barrier is consumed after blocking one hit");
    ...
    assertWithLabel(layeredShield != nullptr && layeredBarrier != nullptr, "compute: create coexistence shield and barrier instances");
    ...
    athena.useItemById(layeredBarrier->getId(), layeredTarget, db);
        assertWithLabel(layeredTarget.hasShield() && layeredTarget.hasBarrier(), "compute: shield and barrier coexist on the same target");
